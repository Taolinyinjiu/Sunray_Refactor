...
PX4_Reader::PX4_Reader(ros::NodeHandle &nh) {
  ...
  try {
    ...
    if (!nh.getParam("uav_id", uav_id)) {
      throw std::runtime_error("无法读取参数 'uav_id'。请检查 YAML "
                               "配置文件是否正确加载到 Parameter Server。");
    }
    if (!nh.getParam("uav_name", uav_name)) {
      throw std::runtime_error("无法读取参数 'uav_name'。请检查 YAML "
                               "配置文件是否正确加载到 Parameter Server。");
    }
...
px4_data::FlightMode
PX4_Reader::flightmode_fromString(const std::string &mode) {
  if (mode == "OFFBOARD")
    return px4_data::FlightMode::OFFBOARD;
  if (mode == "POSCTL")
    return px4_data::FlightMode::POSCTL;
  if (mode == "ALTCTL")
    return px4_data::FlightMode::ALTCTL;
  if (mode == "AUTO.TAKEOFF")
    return px4_data::FlightMode::AUTO_TAKEOFF;
  if (mode == "AUTO.LAND")
    return px4_data::FlightMode::AUTO_LAND;
  if (mode == "AUTO.RTL")
    return px4_data::FlightMode::AUTO_RTL;
  if (mode == "AUTO.MISSION")
    return px4_data::FlightMode::AUTO_MISSION;
  if (mode == "AUTO.LOITER")
    return px4_data::FlightMode::AUTO_LOITER;
  if (mode == "MANUAL")
    return px4_data::FlightMode::MANUAL;
  if (mode == "STABILIZED")
    return px4_data::FlightMode::STABILIZED;
  if (mode == "ACRO")
    return px4_data::FlightMode::ACRO;
};